...
subroutine output_electdouble(countfile)
    ...
    if(verboseflag=="yes") then  
        do i=1,nz   
            write(un_xNa,*)zc(i),xNa(i)
            write(un_xK,*)zc(i),xK(i)
            write(un_Ca,*)zc(i),xCa(i)
            write(un_NaCl,*)zc(i),xNaCl(i)
            write(un_KCl,*)zc(i),xKCl(i)
            write(un_xpair,*)zc(i),(xNaCl(i)/vNaCl)/(xNa(i)/vNa+xCl(i)/vCl+xNaCl(i)/vNaCl)
            write(un_xCl,*)zc(i),xCl(i)
            write(un_psi,*)zc(i),psi(i)
            write(un_charge,*)zc(i),rhoq(i)
...
subroutine output_neutral(countfile)
    ...
    open(unit=un_sys,file=sysfilename)   
    open(unit=un_xpolAB,file=xpolABfilename)
    open(unit=un_xpolC,file=xpolCfilename)
    open(unit=un_xsol,file=xsolfilename)
...
subroutine output_individualcontr_fe(countfile)
    ...
    write(un_xsol,*)'FE              = ',FE  
    write(un_xsol,*)'FEbulk          = ',FEbulk 
    write(un_xsol,*)'deltaFE         = ',deltaFE
    write(un_xsol,*)'FEalt           = ',FEalt  
    write(un_xsol,*)'FEbulkalt       = ',FEbulkalt 
    write(un_xsol,*)'deltaFEalt      = ',deltaFEalt
    
    write(un_xsol,*)"FEtrans%sol     = ",FEtrans%sol   
    write(un_xsol,*)"FEtrans%Na      = ",FEtrans%Na  
    write(un_xsol,*)"FEtrans%Cl      = ",FEtrans%Cl  
    write(un_xsol,*)"FEtrans%Ca      = ",FEtrans%Ca  
    write(un_xsol,*)"FEtrans%K       = ",FEtrans%K
    write(un_xsol,*)"FEtrans%KCl     = ",FEtrans%KCl
    write(un_xsol,*)"FEtrans%NaCl    = ",FEtrans%NaCl  
    write(un_xsol,*)"FEtrans%Hplus   = ",FEtrans%Hplus  
    write(un_xsol,*)"FEtrans%OHmin   = ",FEtrans%OHmin  
    
    write(un_xsol,*)"FEchempot%Na    = ",FEchempot%Na
    write(un_xsol,*)"FEchempot%Cl    = ",FEchempot%Cl
    write(un_xsol,*)"FEchempot%Ca    = ",FEchempot%Ca
    write(un_xsol,*)"FEchempot%K     = ",FEchempot%K
    write(un_xsol,*)"FEchempot%KCl   = ",FEchempot%KCl
    write(un_xsol,*)"FEchempot%NaCl  = ",FEchempot%NaCl
    write(un_xsol,*)"FEchempot%Hplus = ",FEchempot%Hplus
    write(un_xsol,*)"FEchempot%OHmin = ",FEchempot%OHmin

    write(un_xsol,*)"FEchemsurf(LEFT)= ",FEchemsurf(LEFT)
    write(un_xsol,*)"FEchemsurf(RIGHT)= ",FEchemsurf(RIGHT)
    write(un_xsol,*)"FEchemsurfalt(LEFT)= ",FEchemsurfalt(LEFT)
    write(un_xsol,*)"FEchemsurfalt(RIGHT)= ",FEchemsurfalt(RIGHT)

    write(un_xsol,*)"delta FEchemsurfalt(LEFT)= ",FEchemsurfalt(LEFT)-FEchemsurf(LEFT)-diffFEchemsurf(LEFT)
    write(un_xsol,*)"delta FEchemsurfalt(RIGHT)= ",FEchemsurfalt(RIGHT)-FEchemsurf(RIGHT)-diffFEchemsurf(RIGHT)
